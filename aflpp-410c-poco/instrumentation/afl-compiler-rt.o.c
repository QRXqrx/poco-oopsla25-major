...
void __poc_auto_early(void);
static void __poc_map_shm(void);
static void __poc_unmap_shm(void);

// @PoC: debug

#include "debug.h"
static inline void debug_for_poc(char *mes) {
  if (!!getenv("POC_DEBUG")) log_PoC("%s", mes);
}
...
#define POC_CTOR_PRIO   (CTOR_PRIO + 1)
#define POC_AREA_SIZE   (1 << 16)

static u8 __poc_area_init[POC_AREA_SIZE];
static u8 *__poc_area_ptr_dummy = __poc_area_init;

// Runtime ptr
u8 *__poc_area_ptr = __poc_area_init;

// Already initialized flags
u32 __poc_already_initialized_shm;
u32 __poc_already_initialized;

// Lots of variables
u64 __poc_map_addr;

__attribute__((constructor(POC_CTOR_PRIO)))
void __poc_auto_early(void) {

  debug_for_poc("__poc_auto_early()");
...
// @PoC: pass shm_id and attach __poc_area_ptr to the shm.
static void __poc_map_shm(void) {

  debug_for_poc("__poc_map_shm()");

  if (__poc_already_initialized_shm) return;
  __poc_already_initialized_shm = 1;

  // if we are not running in afl ensure the map exists
  if (!__poc_area_ptr) { __poc_area_ptr = __poc_area_ptr_dummy; }

  u8 *id_str = getenv(POC_SHM_ENV_VAR);

  // Read shm_id from env
  if (id_str) {

    debug_for_poc("__poc_map_shm(), id_str");
    debug_for_poc(id_str);

    if (__poc_area_ptr
        && (__poc_area_ptr != __poc_area_init)
        && (__poc_area_ptr != __poc_area_ptr_dummy)) {
    ...
    // Attach __poc_area_ptr to hot cc_shm
    __poc_area_ptr = (u8 *) shmat(shm_id, NULL, 0);
    debug_for_poc("__poc_area_ptr attach success!");

    /* Whooooops. */

    if (!__poc_area_ptr || __poc_area_ptr == (void *)-1) {

      // Exit with abnormal exitcode.
      fprintf(stderr, "shmat() failed for __poc_area_ptr!\n");
      abort();
   ...
   __poc_area_ptr[0] = 1;
   debug_for_poc("set __poc_area_ptr[0] = 1!");
...
static void __poc_unmap_shm(void) {
  ...
  debug_for_poc("__poc_unmap_shm()");

  if (!__poc_already_initialized_shm) return;

  char *id_str = getenv(POC_SHM_ENV_VAR);
  ...
  if (id_str) {
    ...
    shmdt((void *)__poc_area_ptr);

  } else if ((!__poc_area_ptr || __poc_area_ptr == __poc_area_init)) {

    // Exit with abnormal exitcode.
    FATAL("shmdt() failed for __poc_area_ptr!\n");
  ...
  // Reset shm flag.
  __poc_already_initialized_shm = 0;
