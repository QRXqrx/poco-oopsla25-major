...
class ModuleSanitizerCoverageAFL
 ...
 private:
  // @PoC: add module for poc instrument.
...
void ModuleSanitizerCoverageAFL::InjectToggleBeforeBr(
    Module &M, Function &F, ArrayRef<Instruction *> CondBrTargets) {
      ...
      LLVMContext &ctx      = M.getContext();
      std::string togPat    = "TOG_";

      // BasicBlock *prevPostBB = nullptr;
      ...
      for (auto I : CondBrTargets) {
        ...
        if (BranchInst *BRI = dyn_cast<BranchInst>(I)) { 
          ...
          */ 
          ...
          // Make not null comparison.
          Value     *togNotNullCmp = IRB.CreateICmpNE(callGetEnv, nullConst);

          /* Start to add toggle-enabled shortcut jumping */

          // if (prevPostBB) {
          //   errs() << *prevPostBB;
          //   errs() << "=========== [Before split] End of prevPostBB" << '\n';
          // }

          // Get inst's parent BB. 
          // <!> This BB have already included the inst I just created.
          // <!> Don't use BB->splitBasicBlockBefore as it has side effect of changing the dest of its first predcessor.
          BasicBlock *postBB  = I->getParent();                   // This BB stores the original br inst after splitting.
          BasicBlock *predBB  = postBB->splitBasicBlockBefore(I); // This BB stores other insts and add a uncond br to the brBB. <!> side-effect.

          Basic

          // if (prevPostBB) {
          //   errs() << *prevPostBB;
          //   errs() << "=========== [Before insert tog at the end of predBB] End of prevPostBB" << '\n';
          // }
          ...
          // Replace the last uncond br of the predBB into a cond br.
          predBB->getTerminator()->eraseFromParent();                     // Erase (not *remove*) original terminated br.
          BranchInst::Create(origTrueBB, postBB, togNotNullCmp, predBB);  // Create new cond br at the end of predBB. 

          // Refresh the postBB with a newly created br?
          // errs() << *postBB->getTerminator() << "\n";
          // errs() << "==== postBB->getTerminator() ==== \n";
          postBB->getTerminator()->eraseFromParent();
          BranchInst::Create(origTrueBB, origFalseBB, origCond, postBB);
          // BRI->setSuccessor() // TODO: considering calibrate the successors.

          // Instruction *predTermBr = predBB->getTerminator();
          // Instruction *togBr      = BranchInst::Create(origTrueBB, postBB, togNotNullCmp); // Error. Cannot jump directly.
          // Instruction *togBr      = BranchInst::Create(origTrueBB, postBB, togNotNullCmp, predTermBr);
          // predTermBr->eraseFromParent(); // Do not use `removeFromParent()`, or that will leave a <null operand> for its successor
          // errs() << *togBr << "\n";  
          // ReplaceInstWithInst(predTermBr, togBr);

          // if (prevPostBB) {
          //   errs() << *prevPostBB;
          //   errs() << "=========== End of prevPostBB" << '\n';
          // }
