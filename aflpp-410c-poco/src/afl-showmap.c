...
static void showmap_run_target_forkserver(afl_forkserver_t *fsrv, u8 *mem,
  ...
  // log_PoC("fsrv->exec_tmout %u", fsrv->exec_tmout);
  ...
  if (!fsrv->last_run_timed_out && !stop_soon &&
    ...
    // Poc
    // log_PoC("child status: %u", WTERMSIG(fsrv->child_status));    
    child_crashed = true;
  ...
  // log_PoC("now child status is %d",child_crashed);
...
static void showmap_run_target(afl_forkserver_t *fsrv, char **argv) {
  ...
  if (waitpid(fsrv->child_pid, &status, 0) <= 0) { 
    log_PoC("waitpid() failed");
  ...
  if (*(u32 *)fsrv->trace_bits == EXEC_FAIL_SIG) {
    log_PoC("Unable to execute '%s'", argv[0]);
  ...
  if (!fsrv->last_run_timed_out && !stop_soon && WIFSIGNALED(status)) {
    // log_PoC("crash detected : %d", WTERMSIG(status));
    child_crashed = true;
...
u32 execute_testcases(u8 *dir) {
  ...
  log_PoC("dir=%s", dir);
  log_PoC("nl_cnt=%d", nl_cnt);
  log_PoC("alphasort=%d", alphasort);
  ...
  for (i = 0; i < (u32)nl_cnt; ++i) {
    ...
    // @PoC: write coverage for each of the file.
    if (!collect_coverage)
...
int main(int argc, char **argv_orig, char **envp) {
  ...
  // @PoC: Seems a very complex map_size reset and via a single call of afl_frsv_start()
  if (!fsrv->cs_mode && !fsrv->qemu_mode && !unicorn_mode) {
    ...
    // @PoC: Directly start if one of cs_mode, qemu_mode, unicorn_mode
  ...
  // @PoC: configure custimized mutators. 
  if (in_dir || in_filelist) {
  ...
  if (cmin_mode && !!getenv("AFL_CMIN_CRASHES_ONLY")) {
    ...
    //Poc
    // log_PoC("ret : %d , child_crashed : %d",ret,child_crashed);
