...
// @PoC: build shm to read tog flags.
u8 *poc_shm_init(sharedmem_t *shm, size_t size) {

  // Debug
  log_PoC("poc_shm_init()");
  ...
  if (shm->map_size > 0) {
    ...
    if (shm->map_size != size) {

      PFATAL("@PoC, Unequal map_size, cc_map_size %u, size %lu",
             shm->map_size, size);

    } else {

      map_size = shm->map_size;
  ...
  if (shm->shm_id < 0) { // Get checking

    PFATAL("@PoC, poc_shm_init(), shmget() failed");

  }

  // Send shm id through env.
  // Make the hot cc_shm visible to the instrumented PUT
  u8 *id_str = alloc_printf("%d", shm->shm_id);
  setenv(POC_SHM_ENV_VAR, id_str, 1);
  ck_free(id_str);
  ...
  if (shm->map == (void *)-1 || !shm->map) {
    ...
    shmctl(shm->shm_id, IPC_RMID, NULL);  // do not leak shmem
    PFATAL("@PoC, poc_shm_init(), shmctl -- shmat() failed");
...
// @PoC: destroy the shm.
void poc_shm_deinit(sharedmem_t *shm){

  // Debug
  log_PoC("poc_shm_deinit()");
