...
static inline void insert_object(aflcc_state_t *aflcc, u8 *obj, u8 *fmt,
  ...
  if (aflcc->instrument_mode == INSTRUMENT_POC)
    log_PoC("Insert an aflcc_param, `%s`", aflcc->cc_params[aflcc->cc_par_cnt-1]);
...
// @PoC
void     add_poc_pcguard(aflcc_state_t *);
...
static void instrument_mode_new_environ(aflcc_state_t *aflcc) {
  ...
  while (ptr2) {
    ...
    // @PoC
    if (strncasecmp(ptr2, "poc", strlen("poc")) == 0) {
      aflcc->instrument_mode = INSTRUMENT_POC;
    }
...
// @PoC: a modified copy of add_optimized_pcguard
void add_poc_pcguard(aflcc_state_t *aflcc) {

  log_PoC("Ok, now trying to add poc so");
  ...
  if (aflcc->have_instr_list) {
    ...
    insert_param(aflcc, "-fexperimental-new-pass-manager");
    #endif
    insert_object(aflcc, "SanitizerCoveragePoC.so", "-fpass-plugin=%s", 0);
  ...
  if (aflcc->compiler_mode == LLVM || aflcc->compiler_mode == LTO) {
    ...
    if (aflcc->lto_mode) {
      ...
      if (aflcc->instrument_mode == INSTRUMENT_POC) { 
        ...
        add_poc_pcguard(aflcc);

      } else if (aflcc->instrument_mode == INSTRUMENT_PCGUARD) { // @Adian: default instrumentation.
...
int main(int argc, char **argv, char **envp) {
  ...
  // @PoC
  if (aflcc->instrument_mode == INSTRUMENT_POC)
    log_PoC("Seems the PCGUARD-PoC instrumentation is on, yeah!");
